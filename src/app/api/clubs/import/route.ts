...
import { parseClubCsv } from "@/lib/csv/club-import";
import { importClubRecord } from "@/lib/clubs/import-service";
...
export async function POST(request: Request) {
  ...
  const { rows, errors: parseErrors } = parseClubCsv(content);
  ...
  for (const row of rows) {
    try {
      const result = await importClubRecord({
        organizationId: session.user.organizationId,
